...
let
  ...
  sshKeysOfUser = user: let u = getUser user { openssh.authorizedKeys = { keys = []; keyFiles = []; }; }; in getSshKey u;
  ...
  + check (countMembersOf "wheel" usernames > 0) "At least 1 member of wheel group\n"
